...
for (comb in layer_combinations) {
  ...
  if (iterationCount == 1){ coef_df <- getCoefficientDF(modelOutput$ModelBuilding$model$finalModel) } else{
    coef_df <- modelOutput$ModelBuilding$combinedWeights }
  ...
  cancer_summary_table <- rbind(cancer_summary_table, getSummaryTable(cancer_type, comb, modelOutput, stats, params=params))
}
