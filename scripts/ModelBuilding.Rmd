...
bias_correction <- function(predicted_age, true_age) {
  ...
  bias_fit <- lm(predicted_age ~ true_age)
  
  # Extract coefficients
  alpha <- coef(bias_fit)[1]  # Intercept
  beta <- coef(bias_fit)[2]  # Slope
  
  # Correct predictions
  corrected_predicted_age <- predicted_age - (alpha + beta * true_age)
  
  return(corrected_predicted_age)
}
```
...
ModelBuilding <- function(inputList, cancerName="HNSC", splitSize=0.8, testOnCompleteData=FALSE, methodNames=c("glmnet"), graphTitle="", seed=42, pca=FALSE, columnsToKeep=c("Age", "gender", "race", "HPV.status"), stratifying=FALSE, iterationCount=5, applyBiasCorrection=TRUE){
  ...
  for (j in 1:iterationCount){
    ...
    for (k in 1:length(methodNames)){
      ...
      applyBiasCorrection = FALSE  # Bias correction not applied for individual runs in ensemble)
