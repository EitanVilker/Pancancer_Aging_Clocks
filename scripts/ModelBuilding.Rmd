...
```{r}
ModelBuilding <- function(inputList, cancerName="HNSC", splitSize=0.8, testOnCompleteData=FALSE, methodNames=c("glmnet"), graphTitle="", seed=42, pca=FALSE, columnsToKeep=c("Age", "gender", "race", "HPV.status"), stratifying=FALSE, iterationCount=5,applyBiasCorrection=TRUE){
  ...
  # Handle single or multiple model methods
  ...
  if (applyBiasCorrection) {
    ...
    DF_corrected <- data.frame(
      sampleID = rownames(remainingTests),
      true_age = remainingTests$Age,
      pred_age = correctedPredictions
    )

    correctedPlot <- ggplot2::ggplot(DF_corrected, aes(x = true_age, y = pred_age)) +
      geom_point(color = "green") +
      geom_smooth(method = "lm", se = FALSE, aes(color = "Regression Line")) +
      geom_abline(intercept = 0, slope = 1, linetype = "dashed", aes(color = "Identity Line")) +
      scale_color_manual(values = c("Regression Line" = "orange")) +
      labs(title = paste("Corrected Ensemble Predictions -", graphTitle), x = "True Age", y = "Corrected Predicted Age", color = "Legend") +
      labs(caption = paste("R^2: ", toString(round(R2_corrected, 3)), "- RMSE: ", toString(round(RMSE_corrected, 3)))) +
      theme(plot.caption.position = "plot", plot.caption = element_text(hjust = 0))
