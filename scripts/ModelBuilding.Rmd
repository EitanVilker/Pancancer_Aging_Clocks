...
getCleanedAssayData <- function(experimentsList, assayNames, useImputed, simplifiedDataPairs=list(), checkingAllCancers=FALSE){
  ...
  for (i in 1:length(experimentsList)){    
    ...
    experiment <- experimentsList[[i]]
    if (length(simplifiedDataPairs) > 0 && length(simplifiedDataPairs[[i]]) > 1){
      simplifiedData <- simplifiedDataPairs[[i]]$data
      significantFeatures <- simplifiedData[simplifiedData[[simplifiedDataPairs[[i]]$metric]] <= 0.025, ]
    ...
    if (useImputed[[layerName]]){ assayType <- paste(assayNames[[i]],"-imputed",sep="")}
    else { assayType <- assayNames[[i]]}
...
ModelBuilding <- function(inputList, cancerName="HNSC", splitSize=0.8, testOnCompleteData=FALSE, methodNames=c("glmnet"), graphTitle="", seed=42, pca=FALSE, columnsToKeep=c("Age", "gender", "race", "HPV.status"), stratifying=FALSE, iterationCount=5){
  ...
  for (j in 1:iterationCount){
    ...
    for (k in 1:length(methodNames)){
      ...
      # correlations <- c(correlations, cor(predictions$Age, predictions$predicted_age))
    }
  ...
  return(list(model=modelsList, predicted=remainingTests, weights=weightsSurvival))
}
```
...
```{r}
setup2 <- function(setup1List, featuresToEnsure=c("Age")){
  ...
  # Perform imputation
  experimentsList <- imputeMissingValues(experimentsList, assayNames, needsImputation, checkingAllCancers=checkingAllCancers) # Run if missing values in assay
...
```{r}
performSetup <- function(layersVector, cancerName="HNSC", checkingAllCancers=FALSE, featuresToEnsure=c("Age"), removeHPVPositive=FALSE){
  return(setup2(setup1(layersVector, cancerName=cancerName, checkingAllCancers=checkingAllCancers), featuresToEnsure=featuresToEnsure))
}
```
...
```{r}
standardizedModelBuilding <- function(layersVector, cancerName="HNSC", splitSize=0.999, testOnCompleteData=FALSE, graphTitle="", methodNames=c("glmnet"), seed=43, columnsToKeep=c("Age", "gender", "race", "HPV.status", "submitter_id"), stratifying=FALSE){
  if (splitSize > 0.98){ testOnCompleteData=TRUE}
  modelBuildingInput <- performSetup(layersVector, cancerName=cancerName)
...
```{r}
# results <- read.csv("./ModelBuildingOutput/3Models3Layers5BayesLogTest.csv")
predictions <- modelOutput$ModelBuilding$predicted
# df <- data.frame(predicted_age=results$predicted_age, submitter_id=results$submitter_id)
df <- data.frame(predicted_age=predictions$predicted_age, submitter_id=predictions$submitter_id)
reducedExperimentList <- modelOutput$experimentList[[1]][, modelOutput$experimentList[[1]]$submitter_id %in% df$submitter_id]
stats <- run_analysis_pipeline(modelOutput$experimentList[[1]], df)
# stats$interaction_summary$logtest
...
### Find coefficients of individual features
```{r}
## For RNAseq data
