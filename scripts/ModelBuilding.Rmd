  ...
  if (applyBiasCorrection) {
    ...
    if (do_plot && nchar(graphTitle) > 0) {
      DF_after <- data.frame(
        sampleID = rownames(meta_tst),
        true_age = meta_tst$Age,
        pred_age = meta_tst$predicted_corrected_age
      )
      
      g2 <- ggplot2::ggplot(DF_after, aes(x = true_age, y = pred_age)) +
        geom_point(color = "green") +
        geom_smooth(method = "lm", se = FALSE, aes(color = "Regression Line")) +
        geom_abline(intercept = 0, slope = 1, linetype = "dashed", aes(color = "Identity Line")) +
        scale_color_manual(values = c("Regression Line" = "orange")) +
        labs(title = paste("After Bias Correction: ", graphTitle, sep=""), x = "True Age", y = "Corrected Predicted Age", color = "Legend") +
        labs(caption = paste("R^2:", toString(round(R2_after, 3)), "- RMSE:", toString(round(RMSE_after, 3)))) +
