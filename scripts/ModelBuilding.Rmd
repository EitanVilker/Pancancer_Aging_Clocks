...
ModelBuilding <- function(inputList, cancerName="HNSC", splitSize=0.8, testOnCompleteData=FALSE, methodNames=c("glmnet"), graphTitle="", seed=42, pca=FALSE, columnsToKeep=c("Age", "gender", "race", "HPV.status"), stratifying=FALSE, iterationCount=NULL, applyBiasCorrection=FALSE, combiningNormal=FALSE){
  ...
  for (name in names(experimentsList)){
    if (name %in% c("CESC", "OV", "PRAD", "TGCT", "UCEC", "UCS", "BRCA")){ columnsToKeep <- columnsToKeep[! columnsToKeep %in% "gender"] }
  }
  if (any(names(experimentsList) != "HNSC")){ columnsToKeep <- columnsToKeep[! columnsToKeep %in% "HPV.status"]}
  if (cancerName %in% c("CESC", "OV", "PRAD", "TGCT", "UCEC", "UCS", "BRCA")){ columnsToKeep <- columnsToKeep[! columnsToKeep %in% "gender"] }
