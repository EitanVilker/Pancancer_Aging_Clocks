...
ModelBuilding <- function(inputList, cancerName="HNSC", splitSize=0.8, testOnCompleteData=FALSE, methodNames=c("glmnet"), graphTitle="", seed=42, pca=FALSE, columnsToKeep=c("Age", "gender", "race", "HPV.status"), stratifying=FALSE, iterationCount=NULL, applyBiasCorrection=FALSE, combiningNormal=FALSE){
  ...
  ## Cross validation
    ...
    for (i in 1:iterationCount){
      ...
      # return(list(model=model, final=model$model$finalModel, bestTune = model$model$bestTune))
      ...
      for (j in 1:length(featureFrame$features)){
        feature <- featureFrame$features[j]
        coef <- featureFrame$coefficients[j]
        idx <- which(combinedWeights$feature == feature)
        weights[idx] <- coef * modelWeight
      }
      combinedWeights[toString(i)] <- weights
    }
    
    combinedWeights$meanSums <- rowSums(combinedWeights[, -1])
