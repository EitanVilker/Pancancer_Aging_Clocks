...
for (cancer in params$cancer_types) {
  ...
  for (name in names(files)) {
    ...
    if (file.exists(file)) {
      ...
      if (nrow(data) > 0) {
        pathways <- unique(c(pathways, data$ID))  # Combine pathways safely
      }
    } else {
  ...
  if (length(pathways) > 0) {
    significant_pathways_list[[cancer]] <- pathways
  }
}
...
if (length(significant_pathways_list) == 0) {
  ...
  pathway_counts <- rbindlist(
    lapply(names(significant_pathways_list), function(cancer) {
      data.table(Pathway = significant_pathways_list[[cancer]], Cancer_Type = cancer)
    }),
    fill = TRUE
  )

  # Count occurrences of each pathway across cancer types
  pathway_summary <- pathway_counts[, .(Cancer_Types = paste(Cancer_Type, collapse = ", "), Count = .N), by = Pathway]

  # Filter for pathways that appear in two or more cancer types
  common_pathways <- pathway_summary[Count > 1]

  # Save results
  output_path <- file.path(base_path, "common_pathways.csv")
  fwrite(common_pathways, output_path)

  # Print summary
  print("✅ Common pathways identified across multiple cancer types:")
  print(common_pathways)

  # If running interactively, display results in a table
  if (interactive()) {
    library(DT)
    datatable(common_pathways)
  }
}
